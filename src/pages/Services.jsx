      ...
      <div className="max-w-7xl mx-auto px-6 mt-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {WORKFLOW_STEPS.map((item, idx) => (
            <div key={idx} className="relative p-6 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm group">
              <span className="absolute top-4 right-6 text-5xl font-extrabold text-black/10 dark:text-white/10 group-hover:text-black/20 dark:group-hover:text-white/20 transition-colors duration-300 font-sans">
                {item.step}
              </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {PRICING_TIERS.map((tier, idx) => (
            <div 
              key={idx} 
              className={`
                relative p-8 rounded-2xl flex flex-col bg-white dark:bg-black shadow-lg border transition-all duration-300 hover:scale-[1.01]
                ${tier.popular ? "border-black dark:border-white ring-1 ring-black/10 dark:ring-white/10" : "border-slate-200 dark:border-slate-800"}
              `}
            >
              {tier.popular && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-[10px] font-bold text-white dark:text-black bg-black dark:bg-white tracking-wider uppercase shadow-md">
              ...
              <div className="flex items-baseline gap-1 mb-6 border-b border-slate-200 dark:border-slate-800 pb-6">
              ...
              <ul className="space-y-3 flex-1 mb-8" role="list">
                {tier.features.map((feat, fIdx) => (
                  <li key={fIdx} className="flex items-start gap-2.5 text-sm text-slate-600 dark:text-slate-300 font-light">
                    <Check size={14} className="text-black dark:text-white mt-0.5 flex-shrink-0" />
                    <span>{feat}</span>
                  </li>
                ))}
              </ul>
              ...
              <button className={`w-full py-3 text-xs font-semibold rounded-xl tracking-wide cursor-pointer transition-all duration-200 ${
                tier.popular 
                  ? "bg-black text-white shadow-md shadow-black/10 hover:bg-slate-900 dark:bg-white dark:text-black dark:hover:bg-slate-100 dark:shadow-white/10" 
                  : "bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700"
              }`}>
